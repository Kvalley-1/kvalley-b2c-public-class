...
### [2026-05-24] 單元一找碴練習活動稿 — CLAUDE.md/Knowledge/SKILL/MCP 四概念鞏固

> 📌 **狀態：擱置稿，留參考**（2026-05-27 從 `單元一-教案.md` 併入此檔，原檔刪除）
> 單元一設計暫停發展（太複雜、非定稿方向）。活動本身具備教學設計價值，移入 D 段保留參考。
> 完整脈絡見 `WORKING_MEMORY.md` 2026-05-24「教學設計師 session」條目。
...
### 2026-05-27
- **檔名變更**：`claude-code-instruction-design.md` → `教案.md`（短名易打）
- **單元一-教案併入**：D 段新增「單元一找碴練習活動稿」（擱置稿，留參考），原 `單元一-教案.md` 刪除
